...
function loadBook(data, cfi, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor) {
  ...
  rendition.on("relocated", function (location) {
    ...
    // Get HTML of each active content frame
    var pagesHtml = [];
    rendition.getContents().forEach((contents) => {
      pagesHtml.push(contents.document.documentElement.outerHTML);
    });

    // Send both location + HTML together
  ...
  rendition.hooks.content.register((contents) => {
    ...
    const selectors = ["a", "span", "h2"];

    selectors.forEach((selector) => {
      doc.querySelectorAll(selector).forEach((element) => {
        element.addEventListener("click", function (event) {
          event.preventDefault();
          event.stopPropagation(); // stop epub.js from hijacking links
          ...
          console.log("Clicked tag:", clickedInfo); // 🔍 debug log
          window.flutter_inappwebview.callHandler("tagClicked", clickedInfo);
        });
      });
    });
  });
